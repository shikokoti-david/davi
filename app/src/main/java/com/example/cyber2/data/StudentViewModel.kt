...
class StudentViewModel : ViewModel() {
    ...
    fun uploadStudentWithImage(
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    database.child(studentId).setValue(student)
                        ...
                        .addOnFailureListener { exception ->
                            Log.e("FirebaseError", "Failed to save student: ${exception.message}", exception)
                            viewModelScope.launch {
                                withContext(Dispatchers.Main) {
                                    Toast.makeText(
                                        context,
                ...
                Log.e("UploadError", "Exception: ${e.localizedMessage}", e)
                withContext(Dispatchers.Main) {
                    Toast.makeText(
                        context,
    ...
    fun updateStudent(
        ...
        val updatedStudent = StudentModel(
            ...
            summary = summary,
            nationality = nationality,
            imageUrl = "", // Keep this if image update not supported
            studentId = studentId
        )
